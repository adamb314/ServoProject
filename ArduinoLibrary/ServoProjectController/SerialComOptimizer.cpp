...
bool SerialComOptimizer::read(uint8_t& c, uint32_t timeoutInMs)
{
    if (available() == 0)
    {
        ...
        while (true)
        {
            ...
            if (static_cast<int32_t>(millis() - startTime) > timeoutInMs)
            {
                return false;
            }
        }
...
void SerialComOptimizer::collectReadData()
{
    ...
    CppArray<char, 32> tempReadBuffer;
...
void SerialComOptimizer::repeatReceivedToBridges(const CppArray<char, 32>& buffer, int32_t messageLength)
{
    if (messageLength > 0)
    {
...
void SerialComOptimizer::writeBridgedResponse()
{
    ...
    if (availableToBridge != 0)
    {
        ...
        readAmount = stdmin::min(readAmount, sizeLeftInWriteBuffer);
        readAmount = stdmin::min(readAmount, availableToBridge);
